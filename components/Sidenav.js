        ...
        subjects.map(subject => (
          <li key={subject._id} onClick={() => { toggle ? setToggle("") : setToggle(subject._id) }}>
            ...
            <ul className={(toggle === subject._id) ? "py-2 space-y-2 bg-gray-900 rounded-b-md" : "hidden"}>
              ...
              <li>
                <a onClick={() => setActiveComponent(`Tests:${subject.classroom}:${subject.subject}`)} className="flex items-center p-2 pl-11 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-gray-100 dark:text-white dark:hover:bg-gray-700">Tests</a>
              </li>
            </ul>
          </li>
        ))
